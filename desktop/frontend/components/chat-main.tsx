...
/** 首次进入对话框的许可证信息卡片 — 引导流程完成后自动展示一次 */
function PermitWelcomeCard({ onOpenDashboard, onClose }: { onOpenDashboard: () => void; onClose: () => void }) {
  const [enterprise, setEnterprise] = useState<Record<string, string> | null>(null)
  const [permitData, setPermitData] = useState<Record<string, unknown> | null>(null)
  useEffect(() => {
    apiGet<Record<string, string>>('/api/enterprise').then(r => { if (r.ok && r.data) setEnterprise(r.data) }).catch(() => {})
    apiGet<{ ok: boolean; parsed?: Record<string, unknown>; ai?: { compliance_score?: number } }>('/api/permit/data')
      .then(r => { if (r.ok && r.data?.ok) setPermitData(r.data as Record<string, unknown>) }).catch(() => {})
  }, [])

  if (!enterprise?.name) return null
  const parsed = (permitData?.parsed || {}) as Record<string, unknown>
  const outlets = (parsed.emissionOutlets as unknown[] | undefined) || []
  const score = (permitData?.ai as { compliance_score?: number } | undefined)?.compliance_score
  const validTo = enterprise.valid_to || (parsed.validTo as string) || ""
  const days = validTo ? Math.ceil((new Date(validTo).getTime() - Date.now()) / 86400000) : null
  const expiryTone = days === null ? "text-muted-foreground" : days < 0 ? "text-destructive" : days <= 60 ? "text-warning" : "text-success"

  const Row = ({ label, value, cls }: { label: string; value?: string; cls?: string }) => value ? (
    <div className="flex items-center justify-between gap-4 text-body">
      <span className="text-muted-foreground shrink-0">{label}</span>
      <span className={cn("font-medium text-foreground truncate", cls)}>{value}</span>
    </div>
  ) : null

  return (
    <div role="region" aria-label="企业排污许可证信息" className="relative w-full rounded-2xl border border-eco-200 bg-gradient-to-br from-eco-50/80 to-card p-5 shadow-sm animate-in fade-in slide-in-from-bottom-3 duration-500">
      <button onClick={onClose} aria-label="关闭许可证卡片" className="absolute right-3 top-3 rounded-md px-2 py-1 text-caption text-muted-foreground hover:bg-accent hover:text-foreground transition-colors">收起</button>
      <div className="flex items-center gap-2 mb-3">
        <ShieldCheck className="size-4 text-eco-600" strokeWidth={2} />
        <span className="text-body font-semibold text-foreground">已接入企业排污许可证</span>
        {typeof score === "number" && (
          <span className="ml-auto rounded-full bg-eco-100 px-2.5 py-0.5 text-caption font-medium text-eco-700">合规评分 {score}</span>
        )}
      </div>
      <div className="space-y-1.5">
        <Row label="企业名称" value={enterprise.name} />
        <Row label="许可证编号" value={enterprise.permit_number || (parsed.permitNumber as string)} />
        <Row label="行业类别" value={enterprise.industry || (parsed.industryCategory as string)} />
        <Row label="管理类别" value={enterprise.management_level || (parsed.managementLevel as string)} />
        {validTo && (
          <div className="flex items-center justify-between gap-4 text-body">
            <span className="text-muted-foreground shrink-0">有效期至</span>
            <span className={cn("font-medium", expiryTone)}>
              {validTo}{days !== null && (days < 0 ? `（已逾期 ${-days} 天）` : `（剩余 ${days} 天）`)}
            </span>
          </div>
        )}
        {outlets.length > 0 && <Row label="排放口" value={`${outlets.length} 个已识别`} />}
      </div>
      <div className="mt-4 flex gap-2">
        <button onClick={onOpenDashboard}
          className="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-eco-600 px-4 py-2.5 text-body font-medium text-white hover:bg-eco-700 transition-colors">
          <BarChart3 className="size-4" strokeWidth={1.75} /> 查看可视化看板
        </button>
        <button onClick={onClose}
          className="flex-1 rounded-xl border border-border bg-card px-4 py-2.5 text-body font-medium text-foreground hover:bg-accent transition-colors">
          开始对话
        </button>
      </div>
    </div>
...
export function ChatMain({ leftOpen, onToggleLeft }: {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && localStorage.getItem("ecopilot-first-entry") === "1") {
      localStorage.removeItem("ecopilot-first-entry")
      setFirstEntry(true)
    }
  }, [])
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (mod && e.key === "k") {
        e.preventDefault()
        window.dispatchEvent(new CustomEvent("ecopilot:search"))
      }
      // Esc : 停止生成
              ...
              <div className="relative z-10 mt-10 w-full max-w-3xl">
                {firstEntry ? (
                  <div className="mx-auto max-w-md">
                    <PermitWelcomeCard
                      onOpenDashboard={() => dispatch({ type: "SET_NAV", nav: "dashboard" })}
                      onClose={() => setFirstEntry(false)}
                    />
                  </div>
                ) : (
