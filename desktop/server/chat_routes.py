"""EcoPilot API 路由 — 从 chat_core 导入, 注册到 app"""
...
async def health():
    ...
    gateway_ok = True
    if is_omniroute_mode():
        gateway_ok = await check_omniroute_health()
    return {
        ...
        "omniroute_mode": is_omniroute_mode(),
...
async def save_model_config(request: Request):
    """保存模型配置到后端（API Key + 模型选择），写入后立即热重载 AI 客户端"""
    body, err = await parse_json(request)
    if err is not None: return err
    ...
    try:
        ...
        # 热重载 AI 客户端：更新环境变量 → 重建客户端 → 同步到共享状态
        from openai import AsyncOpenAI
        import chat_core
        if text_key: os.environ["DEEPSEEK_API_KEY"] = text_key
        if vision_key: os.environ["KIMI_API_KEY"] = vision_key
        if text_model: os.environ["ECOPILOT_TEXT_MODEL"] = text_model
        if vision_model: os.environ["ECOPILOT_VISION_MODEL"] = vision_model

        chat_core.ds_client = AsyncOpenAI(
            api_key=os.environ.get("DEEPSEEK_API_KEY", ""),
            base_url=os.environ.get("DEEPSEEK_BASE_URL", "https://api.deepseek.com").rstrip("/"),
        )
        chat_core.kimi_client = AsyncOpenAI(
            api_key=os.environ.get("KIMI_API_KEY", ""),
            base_url=os.environ.get("KIMI_BASE_URL", "https://api.moonshot.cn/v1").rstrip("/"),
        )
        chat_core.TEXT_MODEL = os.environ.get("ECOPILOT_TEXT_MODEL", "deepseek-v4-flash").strip()
        chat_core.KIMI_VISION_MODEL = os.environ.get("ECOPILOT_VISION_MODEL", "moonshot-v1-32k-vision-preview").strip()

        log.info(f"AI 客户端热重载完成: text={chat_core.TEXT_MODEL} vision={chat_core.KIMI_VISION_MODEL}")
        return {"ok": True, "detail": "模型配置已保存并即刻生效"}
    except Exception as e:
...
async def enterprise_get():
    return load_enterprise_info() or {}
    ...
    async def _stream():
        try:
            if is_text:
                ...
                yield sse({"type": "progress", "step": 1, "name": "读取档案内容"})
                content = filepath.read_text(encoding="utf-8", errors="replace")
                if len(content) > 30000:
                    content = content[:30000] + "\n... [内容过长，已截断]"
                yield sse({"type": "progress", "step": 2, "name": "AI 分析中"})
                ...
                async for chunk in stream:
                    ...
                    if delta:
                        yield sse({"type": "text_delta", "text": delta})
                        await asyncio.sleep(0)
            elif is_image:
                # 图片：base64 交给 Kimi 视觉
                yield sse({"type": "progress", "step": 1, "name": "读取图片"})
                raw = filepath.read_bytes()
                b64 = base64.b64encode(raw).decode()
                yield sse({"type": "progress", "step": 2, "name": "AI 视觉分析中"})
                ...
                yield sse({"type": "progress", "step": 1, "name": "上传 PDF"})
                try:
                    import io as _io
                    # 1. 上传 PDF 到 Moonshot
                    ...
                    yield sse({"type": "progress", "step": 2, "name": "提取 PDF 文本"})
                    # 2. 获取文件提取的文本内容
                    file_content = await kimi_client.files.retrieve_content(file_id=upload_resp.id)
                    # 3. 交给 DeepSeek 分析
                    yield sse({"type": "progress", "step": 3, "name": "AI 分析中"})
                    ...
                    yield sse({"type": "text_delta", "text": f"⚠️ PDF 分析暂不可用（{e}）。您可以下载文件后在对话中上传图片让我分析，或针对文本类档案使用 AI 分析。"})
            else:
                # Office/压缩包：不支持
                yield sse({"type": "text_delta", "text": f"⚠️ 此文件类型（{ext}）暂不支持 AI 在线分析。\n\n建议：\n1. 下载文件后转换为 PDF 或图片再上传分析\n2. 文本类档案（txt/md/csv）可直接分析\n3. 图片档案（jpg/png）可视觉识别"})
        except Exception as e:
            yield sse({"type": "error", "detail": f"分析失败：{e}"})
        yield sse({"type": "done"})
...
async def vault_auto_classify(
    ...
    async def _stream():
        try:
            ...
            yield sse({"type": "progress", "text": f"正在接收文件：{original_name}（{fmt_size_py(size)}）"})

            # 校验扩展名
            ext = Path(original_name).suffix.lower()
            if ext not in ALLOWED_VAULT_EXT:
                yield sse({"type": "error", "detail": f"不支持的文件类型：{ext}"})
                yield sse({"type": "done"})
                return

            if size > MAX_VAULT_FILE_SIZE:
                yield sse({"type": "error", "detail": f"文件过大（{fmt_size_py(size)}），最大支持 50MB"})
                yield sse({"type": "done"})
                return

            yield sse({"type": "progress", "text": "正在读取文件内容..."})
            ...
            yield sse({"type": "progress", "text": "AI 正在智能识别文件类型和分类..."})
...
请只回复一个 JSON，格式：{{"category": "分类名", "code": "文号（如有）", "desc": "一句话描述文件内容"}}"""
            ...
            try:
                ...
                yield sse({"type": "progress", "text": f"AI 识别失败，使用默认分类「其他」: {e}"})

            yield sse({"type": "progress", "text": f"识别完成：{ai_category}"})
            yield sse({"type": "classified", "category": ai_category, "code": ai_code, "desc": ai_desc, "filename": original_name})
            ...
            yield sse({"type": "progress", "text": f"已归档到「{ai_category}」分类"})
            yield sse({"type": "done", "file": record})

        except Exception as e:
            yield sse({"type": "error", "detail": f"智能识别失败：{e}"})
            yield sse({"type": "done"})
...
async def enterprise_save(request: Request):
    ...
    name = sanitize_input(data.get("name", ""), max_len=100)
    credit_code = sanitize_input(data.get("creditCode") or data.get("credit_code", ""), max_len=18)
    permit_number = sanitize_input(data.get("permitNumber") or data.get("permit_number", ""), max_len=30)
    legal_rep = sanitize_input(data.get("legalRepresentative") or data.get("legal_representative", ""), max_len=50)
    address = sanitize_input(data.get("address", ""), max_len=200)
    phone = sanitize_input(data.get("phone", ""), max_len=20)
    industry = sanitize_input(data.get("industryCategory") or data.get("industry", ""), max_len=50)
    mgmt_level = sanitize_input(data.get("managementLevel") or data.get("management_level", ""), max_len=20)
    province = sanitize_input(data.get("province", ""), max_len=20)
    city = sanitize_input(data.get("city", ""), max_len=20)
    county = sanitize_input(data.get("county", ""), max_len=20)
    valid_from = sanitize_input(data.get("validFrom") or data.get("valid_from", ""), max_len=20)
    valid_to = sanitize_input(data.get("validTo") or data.get("valid_to", ""), max_len=20)
    industry_code = sanitize_input(data.get("industryCode") or data.get("industry_code", ""), max_len=10)
    ...
    for k, v in data.items():
        if k not in sanitized and isinstance(v, str):
            sanitized[k] = sanitize_input(v, max_len=200)
...
async def user_save(request: Request):
    ...
    # 输入清洗 + 长度限制（P1-6 & P1-7）
    sanitized = {
        "name": sanitize_input(data.get("name", ""), max_len=50),
        "role": sanitize_input(data.get("role", ""), max_len=30),
        "phone": sanitize_input(data.get("phone", ""), max_len=11),
    }
    # 保留其他未列出的字段（兼容前端），但同样做基础清洗
    for k, v in data.items():
        if k not in sanitized and isinstance(v, str):
            sanitized[k] = sanitize_input(v, max_len=100)
        elif k not in sanitized:
            sanitized[k] = v
    HERMES_HOME.mkdir(parents=True, exist_ok=True)
...
async def send_sms(request: Request):
    ...
    sms_codes[phone] = (code, time.time(), 0)
    # SMS code logged in dev mode only
...
async def verify_sms(request: Request):
    ...
    if saved_code != code:
        ...
        fail_count += 1
        if fail_count >= 5:
            sms_codes[phone] = (saved_code, ts, fail_count)
            return {"ok": False, "detail": "验证失败次数过多，请 30 分钟后重试"}
        sms_codes[phone] = (saved_code, ts, fail_count)
...
async def permit_quick_login(request: Request):
    ...
    # 422: CAS 登录失败（验证码/账号/密码），不是 API 认证 401
...
async def permit_login_submit(request: Request):
    ...
    if not result.get("ok"):
        # 422: CAS 登录失败（验证码/账号/密码错误），不是 API 认证 401
        return JSONResponse(status_code=422, content=result)
    # 登录成功 → 自动保存账号密码（验证码不保存）
    try:
        from credentials_manager import save_credentials
        save_credentials("permit", username, password)
        print(f"[Credentials] 已自动保存 {username} 的排污许可平台凭证")
    except Exception as e:
        print(f"[Credentials] 保存凭证失败: {e}")
    # 登录成功，session_id 已在 _active_sessions 中，供后续 permit-reading 使用
    return {"ok": True, "session_id": session_id, "detail": "登录成功"}


# ═══ 平台凭证管理（保存账号密码，验证码手动输入）═══
...
async def credentials_save(request: Request):
    ...
    body, err = await parse_json(request)
    if err is not None: return err
    platform_id = body.get("platform_id", "").strip()
    username = body.get("username", "").strip()
    password = body.get("password", "").strip()
    if not platform_id or not username or not password:
        return JSONResponse(status_code=400, content={"ok": False, "detail": "缺少 platform_id/username/password"})
    try:
        from credentials_manager import save_credentials
        ok = save_credentials(platform_id, username, password)
        return {"ok": ok, "detail": "凭证已保存" if ok else "保存失败"}
    except Exception as e:
...
async def credentials_list():
    """列出已保存凭证的平台（不返回密码）"""
    try:
        from credentials_manager import list_platforms
...
async def credentials_login(request: Request):
    """使用已保存凭证 + 用户手动输入的验证码登录平台"""
    body, err = await parse_json(request)
    if err is not None: return err
    platform_id = body.get("platform_id", "").strip()
    captcha = body.get("captcha", "").strip()
    ...
    if not platform_id:
        return JSONResponse(status_code=400, content={"ok": False, "detail": "缺少 platform_id"})

    # 读取已保存的凭证
    try:
        from credentials_manager import get_credentials
        cred = get_credentials(platform_id)
    except Exception as e:
        return JSONResponse(status_code=500, content={"ok": False, "detail": f"读取凭证失败: {e}"})

    if not cred:
        return JSONResponse(status_code=404, content={"ok": False, "detail": "未找到该平台的已保存凭证，请先手动登录一次"})

    username = cred.get("username", "")
    password = cred.get("password", "")

    if platform_id == "permit":
        # 排污许可平台：使用已保存凭证 + 手动验证码
        from permit_scraper import quick_login
        result = await quick_login(username, password, prefer_vision=False)
        if result.get("ok"):
            return {"ok": True, "session_id": result["session_id"], "detail": "登录成功"}
        return JSONResponse(status_code=422, content=result)

    return {"ok": False, "detail": f"不支持的平台: {platform_id}"}


@app.delete("/api/credentials/{platform_id}")
async def credentials_delete(platform_id: str):
    """删除指定平台的已保存凭证"""
    try:
        from credentials_manager import delete_credentials
        ok = delete_credentials(platform_id)
        return {"ok": ok, "detail": "已删除" if ok else "未找到"}
    except Exception as e:
        return JSONResponse(status_code=500, content={"ok": False, "detail": str(e)})
...
async def permit_safari_inspect():
    ...
    async def _stream():
        try:
            ...
            yield sse({"type": "progress", "step": 0, "name": "检查 Safari 会话"})
            try:
                page = _safari("read-page", timeout=5)
            except subprocess.TimeoutExpired:
                yield sse({"type": "error", "detail": "Safari 未响应，请确认 Safari 已打开"})
                return
            ...
            if not url:
                yield sse({"type": "error", "detail": "无法读取 Safari 页面，请确认 Safari 已打开排污许可平台"})
                return

            # Step 1: 验证在排污许可平台且已登录
            yield sse({"type": "progress", "step": 1, "name": "验证登录状态"})
            if "permit.mee.gov.cn" not in url:
                yield sse({"type": "error", "detail": f"Safari 当前不在排污许可平台（当前: {title}），请先打开 permit.mee.gov.cn 并登录"})
                return
            if "cas/login" in url.lower():
                yield sse({"type": "error", "detail": "尚未登录，请在 Safari 中完成排污许可平台登录"})
                return

            # Step 2: 导航到仪表盘并读取（轮询等待，最多 5 秒）
            yield sse({"type": "progress", "step": 2, "name": "读取许可证仪表盘"})
            ...
            # Step 3: 解析仪表盘数据
            yield sse({"type": "progress", "step": 3, "name": "解析许可证信息"})
            data = _parse_permit_dashboard(dashboard_text)

            # Step 4: 读取企业基本信息（轮询等待，最多 5 秒）
            yield sse({"type": "progress", "step": 4, "name": "读取企业基本信息"})
            ...
            if not data.get("enterpriseName"):
                yield sse({"type": "error", "detail": "未能读取到企业名称，请确认 Safari 已登录排污许可平台"})
                return

            yield sse({"type": "done", "parsed": data})

        except subprocess.TimeoutExpired:
            yield sse({"type": "error", "detail": "Safari 操作超时，请确认 Safari 响应正常"})
        except Exception as e:
...
async def permit_dashboard(force: str = ""):
    ...
    自动缓存1小时，传 ?force=1 强制刷新。超时时自动返回缓存。
    """
    import asyncio as _asyncio

    # 只要有缓存就直接返回（不强制刷新时），避免耗时请求阻塞前端
    ...
    # 无缓存时返回平台暂不可用（不阻塞）
    return {"ok": False, "detail": "排污许可平台暂不可用，请稍后重试", "enterprise": {"name": "冷水江钢铁有限责任公司"}, "notifications": [], "_cached": False}

    import re as _re
    try:
        login = await _asyncio.wait_for(
            quick_login("yuanbin", "432502@Bin"),
            timeout=15
        )
    except (Exception, _asyncio.TimeoutError) as e:
        _log.warning(f"[Permit] dashboard 登录失败: {e}")
        if stale_cache:
            stale_cache["_cached"] = True
            stale_cache["_stale"] = True
            return stale_cache
        return {"ok": False, "detail": "外部平台暂时无法访问，请稍后重试"}
    except Exception as e:
        if stale_cache:
            stale_cache["_cached"] = True
            stale_cache["_stale"] = True
    ...
    try:
        ...
        # ── 1. 仪表盘通知 ──
        await _asyncio.wait_for(page.wait_for_timeout(4000), timeout=10)
        dash_text = await _asyncio.wait_for(page.evaluate("document.body.innerText"), timeout=10)
    ...
    try:
        ...
        _log.warning(f"[Permit] dashboard 采集异常: {e}")
        if stale_cache:
            stale_cache["_cached"] = True
            stale_cache["_stale"] = True
            return stale_cache
...
_rect_data: dict[str, list[dict]] = load_json_dict("rectification_tasks.json")  # {enterprise_id: [tasks]}

def _save_rectification_tasks():
    save_json_dict("rectification_tasks.json", _rect_data)
...
async def rectification_tasks(request: Request):
    ...
    if action == "list":
        return {"ok": True, "tasks": _rect_data.get(eid, [])}
    ...
    if action == "add":
        ...
        tasks = _rect_data.setdefault(eid, [])
        tasks.append(task)
    ...
    if action == "delete":
        tid = body.get("taskId", "")
        tasks = _rect_data.get(eid, [])
        _rect_data[eid] = [t for t in tasks if t.get("id") != tid]
        _save_rectification_tasks()
        return {"ok": True}
...
_calendar_tasks: dict[str, list[dict]] = load_json_dict("calendar_tasks.json")  # {enterprise_id: [tasks]}

def _save_calendar_tasks():
    save_json_dict("calendar_tasks.json", _calendar_tasks)
...
_calendar_docs: dict[str, dict] = load_json_dict("calendar_docs.json")  # {docId: ...}

def _save_calendar_docs():
    save_json_dict("calendar_docs.json", _calendar_docs)
...
async def calendar_doc_ai_fill(request: Request):
    ...
    async def stream():
        ...
        try:
            # Step 1: 读取企业信息
            yield sse({"type": "progress", "step": 1, "name": "读取企业信息"})

            enterprise = load_enterprise_info() or {}
            ...
            if is_report:
                yield sse({"type": "progress", "step": 1.5, "name": "汇总台账数据"})
...
"""
            ...
            # Step 2: 构造 AI prompt
            yield sse({"type": "progress", "step": 2, "name": "AI 智能填充中"})
...
请根据上述企业真实信息智能填充所有占位符，直接返回填充后的完整文档。"""
            ...
            yield sse({"type": "error", "detail": f"AI 填充失败: {str(e)}"})
...
async def chat_stream(request: Request):
    ...
    if permit_data and isinstance(permit_data, dict):
        async with sessions_lock:
            session_permit[sid] = permit_data
...
def _build_session_context(sid: str, history: list = None):
    ...
    if sid not in sessions:
        context = get_orchestrator_system_prompt(session_permit.get(sid))
        sessions[sid] = [{"role":"system","content":context}]
        # 后端重启后恢复上下文：注入前端传来的最近对话历史
        if history and isinstance(history, list):
            for h in history[-10:]:
                if isinstance(h, dict) and h.get("role") in ("user","assistant"):
                    sessions[sid].append({"role":h["role"],"content":h.get("content","")})
    ...
    for round_idx in range(MAX_TOOL_ROUNDS):
        ...
        async for chunk in resp:
            ...
            if delta.content:
                ...
                yield sse({"type":"text_delta","text":delta.content})
        if tool_calls_acc:
            ...
            for k in sorted(tool_calls_acc.keys()):
                ...
                yield sse({"type":"tool_call","name":fn_name,"args":fn_args})
                result = await execute_tool(fn_name, fn_args, sid)
                yield sse({"type":"tool_result","name":fn_name,"result":result[:200]})
                log_tools_used.append({"name": fn_name, "result": result[:200]})
                async with sessions_lock:
                    sessions[sid].append({"role":"tool","tool_call_id":tc["id"],"content":result})
            ...
            async with sessions_lock:
                sessions[sid].append(assistant_final)
            log_ai_reply[0] = text
            break
        if round_idx == MAX_TOOL_ROUNDS - 1:
            async with sessions_lock:
                sessions[sid].append({"role":"user","content":"请根据以上所有工具执行结果，给出最终回答。"})
    else:
...
async def _run(sid: str, msg: str, image_b64: str = "", saved_attachments: list = None, history: list = None):
    try:
        ...
        if _is_hermes_engine() and _hermes_trigger:
            yield sse({"type": "tool_start", "text": "✈️ Pilot 合规管家思考中..."})
            _hermes_tools = [
            ...
            for name, label in _hermes_tools:
                yield sse({"type": "tool_call", "name": label, "args": ""})
            engine = _get_hermes_engine()
            full_text = await engine.chat(msg)
            for name, label in _hermes_tools:
                yield sse({"type": "tool_result", "name": label, "result": "done"})
            yield sse({"type": "tool_start", "text": ""})
            chunk_size = 200
            for i in range(0, len(full_text), chunk_size):
                yield sse({"type": "text_delta", "text": full_text[i:i+chunk_size]})
                await asyncio.sleep(0.02)
            return

        # ── 首次对话自动启动：并行调4个工具获取合规快照（DeepSeek 模式） ──
        async with sessions_lock:
            is_first_msg = len(sessions[sid]) == 2  # system + 首条用户消息
        if is_first_msg:
            ...
            async with sessions_lock:
                sessions[sid].append({"role":"user","content":ctx})

        yield sse({"type":"tool_start","text":"AI 正在分析，准备调用工具..."})
        ...
        if hermes_result.get("hermes_enhanced"):
            ...
            if hermes_result.get("enhanced_prompt"):
                async with sessions_lock:
                    sessions[sid][-1]["content"] = hermes_result["enhanced_prompt"]
            # 缓存命中：直接返回缓存结果，不走 AI
            if hermes_result.get("cache_hit") and hermes_result.get("cached_result"):
                ...
                yield sse({"type":"text_delta","text":cached_text})
                # 保存会话上下文到记忆层
                try:
            ...
            if agent_info and agent_info.get("agent_info"):
                yield sse({"type":"hermes_agent","agent":agent_info["agent"],"name":agent_info["agent_info"]["name"]})

        # 网关健康检查：OmniRoute 模式下先确保网关存活
        if not await ensure_ai_gateway():
            yield sse({"type":"text_delta","text":"⚠️ AI 网关暂时不可用，已尝试自动重启但未恢复，请稍后重试或检查 OmniRoute 服务。"})
            return
        ...
        if "402" in err_text or "Insufficient" in err_text or "balance" in err_text.lower():
            ...
            yield sse({"type":"text_delta","text":f"⚠️ {friendly}"})
            recovered = await restart_omniroute()
            if recovered:
                yield sse({"type":"text_delta","text":"AI 网关已恢复，请重新发送您的问题。"})
            else:
                yield sse({"type":"error","text":"AI 网关暂时不可用，自动重启未成功。请检查 OmniRoute 服务后重试。"})
            return
        else:
            friendly = f"AI 服务暂时不可用（{err_type}），请稍后重试或联系管理员"
        yield sse({"type":"error","text":friendly})
    finally:
        yield sse({"type":"done"})
...
async def _run_vision(sid: str, msg: str, image_b64: str):
    ...
    try:
        ...
        if not full:
            yield sse({"type":"text_delta","text":"未能识别图片内容"})
            return

        # Phase 2: DeepSeek 文本回复（带工具调用 + 思考模式兼容）
        async with sessions_lock:
            sessions[sid].append({"role":"user","content":f"[图片识别结果]\n{full}\n\n请基于以上信息回答用户问题。"})
        yield sse({"type":"tool_start","text":"正在分析图片内容..."})
        ...
        for round_idx in range(MAX_TOOL_ROUNDS):
            ...
            for tc in msg_obj.tool_calls:
                ...
                if _consecutive_searches >= 3:
                    async with sessions_lock:
                        sessions[sid].append({"role":"user","content":"⚠️ 已连续搜索 {} 次仍未找到精确结果。请基于已有知识库内容直接回答，告知用户当前知识库中未收录该信息，并建议通过 vault_guide 补充档案。不要再次调用 knowledge_search。".format(_consecutive_searches)})
...
async def _err(m): yield sse({"type":"error","text":m}); yield sse({"type":"done"})
...
async def _call_hermes_send(platform: str, target: str, message: str, subject: str = "") -> dict:
    ...
    """
    # 始终使用 EcoPilot 仓库内置的 hermes-agent，不依赖 PATH
    _repo_root = Path(__file__).resolve().parent.parent.parent
    _hermes_python = _repo_root / "hermes-agent" / ".venv" / "bin" / "python"
    _hermes_script = _repo_root / "hermes-agent" / "hermes"
    if _hermes_python.is_file() and _hermes_script.is_file():
        cmd = [str(_hermes_python), str(_hermes_script), "send", "--to", f"{platform}:{target}", "--json", "-q"]
    else:
        return {
            "ok": False,
            "error": "EcoPilot 内置 hermes-agent 未找到。请先初始化 hermes-agent 子模块。",
            "stderr": "hermes-agent not found",
        }
    if subject:
...
async def list_memories(request: Request):
    ...
    try:
        ...
        return cors_json(500, f"读取合规记忆失败: {e}", request)
...
async def list_journals(request: Request):
    ...
    try:
        ...
        return cors_json(500, f"读取工作日志列表失败: {e}", request)
