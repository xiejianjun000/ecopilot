'''EcoPilot shared application core — state, helpers, AI clients'''
"""
...
【输出】简洁优先，结论先行。不超过6段。表格适度使用（排放口清单用表格，少量对比项用文字表述）。emoji 可用于区分信息类别。不写操作步骤编号、不写兜底话术。
